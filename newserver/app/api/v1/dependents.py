...
def create_dependent(
    ...
    dep = Dependent(
        ...
        name=payload.name,
        birth_date=payload.birth_date if payload.birth_date else None,
        sex=payload.sex or "U",
        preferred_call_time=payload.preferred_call_time,
        retry_count=payload.retry_count or 3,
        retry_interval_min=payload.retry_interval_min or 10
    )
    db.add(dep)
    db.commit()
    db.refresh(dep)
...
def list_dependents(db: Session = Depends(get_db), user: User = Depends(require_caregiver)):
    deps = (
        db.query(Dependent)
        .filter(Dependent.caregiver_id == user.id, Dependent.deleted_at.is_(None))
        .order_by(Dependent.id.desc())
        .all()
    )
...
def get_dependent(dep_id: int, db: Session = Depends(get_db), user: User = Depends(require_caregiver)):
    dep = (
        db.query(Dependent)
        .filter(Dependent.id == dep_id, Dependent.caregiver_id == user.id)
        .first()
    )
    if not dep:
...
def delete_dependent(dep_id: int, db: Session = Depends(get_db), user: User = Depends(require_caregiver)):
    ...
    dep.deleted_at = _dt.datetime.utcnow()
    db.commit()
    return {"success": True}
